<?php
    ...
    class Papas extends Model{
        ...
        public function validarNumero($precio){
            if(!isset($precio)) throw new validarExcepcion ('error7- el precio no tiene valor');     //validar que exista
        ...
        public function Precio($idpapas){
            $this->db->query("SELECT precio FROM papas WHERE idpapas = $idpapas ");       //toma el query
            $pre = $this->recorrerPrecio($this->db->fetch());       //lo hace array, pasa a recorrer precio
            return $pre;    //devuelve el precio
        }

        public function recorrerPrecio($papas){           //recorrer precio devuelve solo el valor del precio
            return $papas['precio'];
        }
